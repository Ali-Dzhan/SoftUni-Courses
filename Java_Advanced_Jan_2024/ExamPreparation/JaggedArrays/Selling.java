...
public class Selling {
    ...
    public static void main(String[] args) {
        ...
        while(money < 50 && isWithinBakery){
            ...
            switch (command) {
                case "up" -> isWithinBakery = move(startRow - 1, startCol, matrix);
                case "down" -> isWithinBakery = move(startRow + 1, startCol, matrix);
                case "left" -> isWithinBakery = move(startRow, startCol - 1, matrix);
                case "right" -> isWithinBakery = move(startRow, startCol + 1, matrix);
            }
        }
